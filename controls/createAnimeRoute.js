...
const AnimeOctober = require('../models/animeOctober')
...
// เพื่มเดือนตุลาคม
exports.AddanimeOctober = async (req, res) => {
    const usersesstion = req.session.userlogin;
    try {
        const newAnime = new AnimeOctober({
            nameAnime: req.body.nameAnime,
            Produced: req.body.Produced,
            manuscript: req.body.manuscript,
            episodes: req.body.episodes,
...
exports.EditanimeOctober = async (req, res) => {
    const update_id = req.body.update_id;
    try {
        const animeBord = await AnimeOctober.findOne({ _id: update_id });
        if (!animeBord) {
            return res.status(404).json({ error: "AnimeBord not found" });
        }
...
// October Delete
exports.DeleteAnimeOctober = async (req, res) => {
    try {
        const usersesstion = req.session.userlogin;
        const AnimeMayToDelete = await AnimeOctober.findById(req.params.id);

        if (!AnimeMayToDelete) {
            return res.status(404).send('AnimeOctober not found');
        }

        await AnimeOctober.findByIdAndDelete(req.params.id, { useFindAndModify: false });
        res.redirect(`/edit/anime/boards`);
    } catch (err) {
        console.error(err);
        res.status(500).send('Internal Server Error');
